...
exports = module.exports = internals.Lout = function (config) {
    ...
    Hoek.assert(config.indexTemplate || config.indexTemplatePath, 'Config needs to specify either a indexTemplate or a indexTemplatePath');
    Hoek.assert(config.routeTemplate || config.routeTemplatePath, 'Config needs to specify either a routeTemplate or a routeTemplatePath');
...
internals.Lout.prototype.generateRoutesMarkup = function(routes, docsIndex) {

    Hoek.assert(NodeUtils.isArray(routes), 'routes must be an array');

    var templateData = internals.getRoutesData(routes);
    templateData.path = routes[0].path;
    templateData.docsIndex = docsIndex;
