...
internals.getParamsData = function (param, name, typeName) {
    ...
    const originalType = param.type;
    let type;
    if (param.valids && param.valids.some(Joi.isRef)) {
        type = 'reference';
    }
    else if (param.valids && param.valids.length === 1) {
        // Simplify the single valid case into a constant display, i.e. {param: "value"}
        type = internals.getExistsValues(type, param.valids);
    }
    else if (!name && param.valids) {
        // Used for array and alternatives rendering
        type = internals.getExistsValues(type, param.valids);
        name = 'one of';
    }
    else if (param.valids) {
        type = `one of ${internals.getExistsValues(type, param.valids)}`;
    }
    else {
        type = param.type;
    }
    ...
    const data = {
        ...
        unit: param.unit,
        type,
        disallowedValues: param.invalids ? internals.getExistsValues(originalType, param.invalids) : null,
