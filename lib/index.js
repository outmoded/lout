    ...
    return {
        ...
        handler(request, h) {
            ...
            if (request.query.path && request.query.server) {
                return h.view(settings.routeTemplate, internals.getRoutesData(routing));
            }

            return h.view(settings.indexTemplate, internals.getRoutesData(routing));
        },
        plugins: {
            lout: false
        }
...
internals.processAuth = (route) => {
    ...
    const auth = route.settings.auth.lookup(route);
