...
exports.register = function (server, options, next) {
    ...
    var cssBaseUrl = settings.endpoint + '/css';
    var cssViewUrl = cssBaseUrl;
    if (settings.endpoint.substr(-1) === '/') {
        cssBaseUrl = settings.endpoint.substr(0,settings.endpoint.length - 1) + '/css';
        cssViewUrl = '' + '/css';
    }
    ...
    server.views({
        ...
        runtimeOptions: {
            data: {
                cssBaseUrl: cssViewUrl.replace(/(.*?)((\/\w+)?\/css)/, '$2')
            }
        }
    });
