    ...
    return {
        ...
        handler(request, h) {
            ...
            if (request.query.path && request.query.server) {
                return h.view(settings.routeTemplate, internals.getRoutesData(connections[0].table));
            }

            return h.view(settings.indexTemplate, internals.getConnectionsData(connections));
        },
        plugins: {
            lout: false
        }
...
internals.getConnectionsData = (connections) => {
