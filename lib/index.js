...
exports.register = function (pack, options, next) {
    ...
    pack.route({ method: 'GET', path: settings.endpoint, config: internals.docs(settings) });
    ...
    return {
        ...
        handler: function (request) {
            ...
            if (request.query.path) {
                ...
                return request.reply.view(settings.routeTemplate, templateData).send();
            }

            return request.reply.view(settings.indexTemplate, internals.getRoutesData(routes)).send();
        },
        plugins: {
            lout: false
        }
