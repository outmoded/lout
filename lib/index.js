...
internals.docs = function (settings, server) {
    ...
    return {
        ...
        handler: function (request, reply) {
            ...
            var routingTable = request.server.table();
            var connections = Object.keys(routingTable);
            var model = {};

            connections.forEach(function (connection) {

                if (request.query.server && connection !== request.query.server) {
                    return;
                }

                model[connection] = routingTable[connection].filter(function(item) {
            ...
            if (Object.keys(model).every(function (connection) { return !model[connection].length; })) {
                return reply(Boom.notFound());
            }

            if (request.query.path && request.query.server) {
                var routes = model[request.query.server];
            ...
            return reply.view(settings.indexTemplate, internals.getConnectionsData(model));
        },
        plugins: {
            lout: false
        }
...
internals.getConnectionsData = function (routingTable) {

    var connections = Object.keys(routingTable);

    connections.forEach(function (connection) {

        routingTable[connection] = internals.getRoutesData(routingTable[connection]);
    });
