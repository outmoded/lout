...
internals.docs = (settings, server) => {
    ...
    return {
        ...
        handler(request, h) {
            ...
            routingTable = routingTable.filter((item) => {
            ...
            if (request.query.path && request.query.server) {
                return h.view(settings.routeTemplate, internals.getRoutesData(routingTable));
            }

            return h.view(settings.indexTemplate, internals.getRoutesData(routingTable));
        },
        plugins: {
            lout: false
        }
